...
struct up_dev_s
{
  rspinlock_t lock;     /* Per-UART recursive spinlock (serialization) */
 ...
 *   own recursive spinlock so the RX/TX paths serialize against this UART.
 *   nopreempt additionally holds the scheduler so the holder cannot migrate

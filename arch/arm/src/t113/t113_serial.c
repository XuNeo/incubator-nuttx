/****************************************************************************
 * arch/arm/src/t113/t113_serial.c
 *
...
/* SCLK is the UART input clock (APB1 bus clock).
 * Frequency is defined centrally in hardware/t113_clk.h.
 */
...
#ifdef CONFIG_T113_UART4
static char g_uart4rxbuffer[CONFIG_UART4_RXBUFSIZE];
static char g_uart4txbuffer[CONFIG_UART4_TXBUFSIZE];
#endif

#ifdef CONFIG_T113_UART5
...
#ifdef CONFIG_T113_UART4
static uint8_t g_uart4_rxbounce[UART_DMA_BOUNCE_SIZE] aligned_data(64);
static uint8_t g_uart4_txbounce[UART_DMA_BOUNCE_SIZE] aligned_data(64);
#endif

#ifdef CONFIG_T113_UART5
static uint8_t g_uart5_rxbounce[UART_DMA_BOUNCE_SIZE] aligned_data(64);
static uint8_t g_uart5_txbounce[UART_DMA_BOUNCE_SIZE] aligned_data(64);
#endif
...
/* This describes the state of the T113-S3 UART4 port. */

#ifdef CONFIG_T113_UART4
static struct up_dev_s g_uart4priv =
{
  .uartbase       = T113_UART_VADDR(4),
...
#ifdef CONFIG_T113_UART_DMA
  .drq            = DRQ_UART4_RX,
  .rxbounce       = g_uart4_rxbounce,
  .txbounce       = g_uart4_txbounce,
#endif
};
...
/* This describes the state of the T113-S3 UART5 port. */

#ifdef CONFIG_T113_UART5
static struct up_dev_s g_uart5priv =
{
  .uartbase       = T113_UART_VADDR(5),
...
#ifdef CONFIG_T113_UART_DMA
  .drq            = DRQ_UART5_RX,
  .rxbounce       = g_uart5_rxbounce,
  .txbounce       = g_uart5_txbounce,
#endif
};
...
#    define TTYS0_DEV           g_uart3port /* UART3 is ttyS0 */
#    define UART3_ASSIGNED      1
#  elif defined(CONFIG_T113_UART4)
#    define TTYS0_DEV           g_uart4port /* UART4 is ttyS0 */
#    define UART4_ASSIGNED      1
#  elif defined(CONFIG_T113_UART5)
#    define TTYS0_DEV           g_uart5port /* UART5 is ttyS0 */
#    define UART5_ASSIGNED      1
#  endif
#endif
...
#  define UART3_ASSIGNED      1
#elif defined(CONFIG_T113_UART4) && !defined(UART4_ASSIGNED)
#  define TTYS1_DEV           g_uart4port /* UART4 is ttyS1 */
#  define UART4_ASSIGNED      1
#elif defined(CONFIG_T113_UART5) && !defined(UART5_ASSIGNED)
...
#if defined(CONFIG_T113_UART4) && !defined(UART4_ASSIGNED)
...
#ifdef CONFIG_T113_UART4
static inline void t113_uart4config(void)
{
#ifndef CONFIG_UART4_SERIAL_CONSOLE
  /* Cycle reset, barrier, then deassert + enable clock gate */

  t113_ccu_modify(T113_CCU_UART_BGR, (1 << 20), 0);
  UP_DSB();
  t113_ccu_modify(T113_CCU_UART_BGR, 0, (1 << 20) | (1 << 4));

  /* Configure I/O pins (2-wire only) */

  t113_gpio_config(T113_UART4_TX);
  t113_gpio_config(T113_UART4_RX);
#endif
}
#endif

#ifdef CONFIG_T113_UART5
static inline void t113_uart5config(void)
{
#ifndef CONFIG_UART5_SERIAL_CONSOLE
  /* Cycle reset, barrier, then deassert + enable clock gate */

  t113_ccu_modify(T113_CCU_UART_BGR, (1 << 21), 0);
  UP_DSB();
  t113_ccu_modify(T113_CCU_UART_BGR, 0, (1 << 21) | (1 << 5));

  /* Configure I/O pins (2-wire only) */

  t113_gpio_config(T113_UART5_TX);
  t113_gpio_config(T113_UART5_RX);
#endif
}
#endif
...
static inline uint32_t t113_uartdl(uint32_t baud)
{
  ...
  /* Derive the divisor from the live APB1 rate rather than the compile-time
   * T113_UART_CLK.  On a normal boot clk_init() has already programmed APB1
   * to its design rate before t113_lowsetup() runs, so this reads back the
   * same value the constant would have given; under a foreign master (Linux
   * remoteproc) that owns the shared APB1 clock it reads whatever rate the
   * master left, so the baud is correct without reprogramming the clock.
   */

  uint32_t uart_clk = t113_apb1_freq();
  uint32_t dl     = T113_UART_DL_RTN(uart_clk, baud);
  uint32_t actual;
  uint32_t err;
...
void t113_serial_dma_poll(void)
{
   ...
   * (channels 0-7) and deliver any bytes the DMAC has already moved into
   * the bounce buffer that the framework has not yet picked up.
   */
...
#endif
#ifdef CONFIG_T113_UART4
  t113_dma_rx_poll_dev(&g_uart4port);
#endif
#ifdef CONFIG_T113_UART5
  t113_dma_rx_poll_dev(&g_uart5port);
#endif
}
